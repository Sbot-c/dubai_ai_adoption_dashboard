...
tab1, tab2, tab3 = st.tabs([
    "📈 Workforce Skilling Curve (Human Capital)", 
    "🎯 Portfolio Allocations (Operational Discovery)", 
    "🔗 Strategic Talent Pipelines (External Integrations)"
])
...
with tab3:
    ...
    if not collab_df.empty:
        sources = [str(x) for x in collab_df["Academic_Partner"].tolist()]
        midpoints = [str(x) for x in collab_df["Collaboration_Program"].tolist()]
        targets = [str(x) for x in collab_df["Division"].tolist()]
        ...
        fig_sankey = go.Figure(data=[go.Sankey(
            ...
            link=dict(
                ...
                target=link_target,
                value=link_value,
                color="rgba(197, 160, 89, 0.25)" # Highly elegant golden transparency
            )
        )])
...
st.markdown("### 🔍 Enterprise Project Audit Registry")
st.markdown("""
